...
Modes:
    scriptbook  -> rotkeeper-scriptbook-full.md
    docbook     -> rotkeeper-docbook.md + docbook-clean.md
    configbook  -> rotkeeper-configbook.md
    all         -> all three in order (default)

Usage:
    rotkeeper bookflow [--mode scriptbook|docbook|configbook|all]
                       [--include-reports] [--strip-frontmatter]
                       [--dry-run] [--verbose]
"""
...
def add_parser(subparsers: argparse._SubParsersAction) -> None:
    ...
    p.add_argument(
        "--include-reports", action="store_true", default=False,
        help="Pass to configbook: include large generated reports for a full backup",
    )
    p.add_argument(
        "--strip-frontmatter", action="store_true", default=False,
        help="Pass to docbook: strip YAML frontmatter from included Markdown files",
    )
    p.add_argument("--dry-run",  action="store_true", default=False)
    p.add_argument("--verbose",  action="store_true", default=False)
    p.add_argument("--config",   type=str, default=None, help="Path to user-config.yaml")
...
def run(args: argparse.Namespace, ctx: RunContext | None = None) -> int:
    ...
    all_steps: list[tuple[str, object]] = [
        ("scriptbook", lambda: scriptbook.run(_ns(dry_run=dry, verbose=verbose), ctx)),
        ("docbook",    lambda: docbook.run(_ns(dry_run=dry, verbose=verbose, mode="both", strip_frontmatter=strip), ctx)),
        ("configbook", lambda: configbook.run(_ns(dry_run=dry, verbose=verbose, include_reports=include), ctx)),
    ]

    steps = all_steps if mode == "all" else [(n, fn) for n, fn in all_steps if n == mode]
