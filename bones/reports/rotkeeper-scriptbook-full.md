...
class Config:
    ...
    def GENERATED_CONTENT_DIR(self) -> Path:
        logger.warning("GENERATED_CONTENT_DIR is deprecated; use REPORTS_DIR / 'sitemap' instead")
        return self.REPORTS_DIR / "sitemap"
    ...
    def _apply(self, data: dict) -> None:
        ...
        if "GENERATED_CONTENT_DIR" in data:
            logger.warning("GENERATED_CONTENT_DIR in config is deprecated and will be ignored")
...
def add_parser(subparsers: argparse._SubParsersAction) -> None:
    p = subparsers.add_parser(
        "book",
        help="Binder ritual: bundle scripts, docs, configs, content"
    )
    p.add_argument(
        "--mode",
        choices=[
...
def run(args: argparse.Namespace, ctx: RunContext | None = None) -> int:
    dry   = bool(getattr(args, "dry_run", False)) or (getattr(ctx, "dry_run", False) if ctx else False)
    strip = bool(getattr(args, "strip_frontmatter", False))

    cfg = ctx.config if (ctx and getattr(ctx, "config", None) is not None) else CONFIG

    reports = cfg.REPORTS_DIR
    mode    = args.mode

    logging.debug("book: mode=%s reports=%s content_dir=%s dry=%s",
                  mode, reports, cfg.CONTENT_DIR, dry)
...
def _scriptbook_full(reports: Path, strip: bool, cfg=CONFIG) -> None:
    ...
    for f in sorted(src_root.rglob("*.py")):
        ...
        count += 1
    logging.info("book: scriptbook-full -> %s (%d files)", out, count)


def _docbook(reports: Path, strip: bool, cfg=CONFIG) -> None:
    out = reports / "rotkeeper-docbook.md"
    _write_header(out, "Rotkeeper Docbook",
                  "All markdown documentation with path markers")
    content_dir = cfg.CONTENT_DIR
    if not content_dir.exists():
        logging.warning("book: docbook: content dir not found: %s", content_dir)
        return
    count = 0
    for f in sorted(content_dir.rglob("*.md")):
        ...
        count += 1
    logging.info("book: docbook -> %s (%d files)", out, count)
...
def _docbook_clean(reports: Path, strip: bool, cfg=CONFIG) -> None:
    ...
    content_dir = cfg.CONTENT_DIR
    if not content_dir.exists():
        logging.warning("book: docbook-clean: content dir not found: %s", content_dir)
        return
    count = 0
    with out.open("a", encoding="utf-8") as fh:
        for f in sorted(content_dir.rglob("*.md")):
            ...
            fh.write("\n\n")
            count += 1
    logging.info("book: docbook-clean -> %s (%d files)", out, count)
...
def _contentbook(reports: Path, strip: bool, cfg=CONFIG) -> None:
    ...
    _write_header(out, "Rotkeeper Contentbook",
                  "All markdown in content/ with path markers")
    content_dir = cfg.CONTENT_DIR
    if not content_dir.exists():
    ...
    for f in sorted(content_dir.rglob("*.md")):
        ...
        _append_file_block(out, rel, content)
        count += 1
    logging.info("book: contentbook -> %s (%d files)", out, count)
...
def run(args: argparse.Namespace, ctx: RunContext) -> int:
    ...
    for src in markdown_files:
        ...
        try:
            ...
            sidecar_path = reports_dir / "sitemap" / "sidecars" / rel.with_suffix(".rk.yaml")
...
class SitemapPipeline:
    ...
    def write_index_pages(self) -> None:
        ...
        generated_dir = self.reports_dir / "sitemap"
        if self.dry_run:
    ...
    def write_sidecar_metadata(self) -> None:
        ...
        for page in self.pages:
            ...
            sidecar_path = self.reports_dir / "sitemap" / "sidecars" / Path(page["source"]).with_suffix(".rk.yaml")
...
<!-- START: rc/tests/test_config.py -->

import pytest
from pathlib import Path
from rotkeeper.config import Config  # ← Now just rotkeeper.* !
...
def test_config_defaults(config):
    assert config.BASEDIR.resolve() == Path.cwd().resolve()
    assert str(config.BONES).endswith("bones")

def test_config_apply(tmp_path, config):
    overrides = {"HOME": str(tmp_path / "fakehome")}
    config._apply(overrides)
    assert config.HOME == tmp_path / "fakehome"

def test_reports_dir_override(tmp_path, config):
    config.reportsdir = tmp_path / "reports"
    assert config.REPORTSDIR == tmp_path / "reports"
<!-- END: rc/tests/test_config.py -->

<!-- START: rc/tests/test_render.py -->

import pytest
from pathlib import Path
from datetime import datetime
from rc.rotkeeper.lib.render import file_needs_render, load_render_state, save_render_state
from rc.rotkeeper.config import Config  # Mock if needed
...
def test_file_needs_render_new_dest(tmp_src, tmp_dest):
    assert file_needs_render(tmp_src, tmp_dest, None, {})  # No dest → render

def test_file_needs_render_src_mtime_changed(tmp_src, tmp_dest, mocker):
    mocker.patch("rotkeeper.lib.render.compute_file_mtime", return_value=123.0)
    state = {str(tmp_src): {"src_mtime": 122.0}}
    assert file_needs_render(tmp_src, tmp_dest, None, state)

def test_file_needs_render_template_changed(tmp_src, tmp_dest, mocker):
    template = Path("/fake/template.html")
    mocker.patch("rotkeeper.lib.render.compute_file_mtime", side_effect=[123.0, 456.0])
    state = {str(tmp_src): {"src_mtime": 123.0, "template_mtime": 123.0}}
    assert file_needs_render(tmp_src, tmp_dest, template, state)

def test_load_render_state_missing(tmp_path):
    assert load_render_state(tmp_path / "missing.yaml") == {}

def test_save_render_state(tmp_path):
    state = {"test": {"src_mtime": 123.0}}
    path = tmp_path / "render-state.yaml"
    save_render_state(path, state)
    assert path.exists()
    loaded = load_render_state(path)
    assert loaded == state
<!-- END: rc/tests/test_render.py -->

<!-- START: rc/tests/test_sitemap_pipeline.py -->

import pytest
from pathlib import Path
from rc.rotkeeper.config import Config
from rc.rotkeeper.lib.render import file_needs_render, load_render_state, save_render_state
from rc.rotkeeper.lib.sitemap_pipeline import SitemapPipeline
...
@pytest.fixture
def pipeline(sample_md, tmp_path):
    CONFIG.CONTENTDIR = tmp_path
    return SitemapPipeline(dry_run=False)

def test_collect_pages_filters_drafts(pipeline, sample_md, tmp_path):
    draft_md = tmp_path / "draft.md"
    draft_md.write_text("---\ndraft: true\n---")
    pages = pipeline.collect_pages()
    assert len(pages) == 1  # Only sample_md
    assert pages[0]["title"] == "Test Page"

def test_write_sidecar_metadata(pipeline, sample_md):
    pipeline.run(["collect-pages", "write-sidecar-metadata"])
    sidecar = sample_md.with_suffix(".rk.yaml")
    assert sidecar.exists()
    data = yaml.safe_load(sidecar.read_text())
    assert "rotkeeper" in data
    assert "breadcrumb" in data["rotkeeper"]
<!-- END: rc/tests/test_sitemap_pipeline.py -->
